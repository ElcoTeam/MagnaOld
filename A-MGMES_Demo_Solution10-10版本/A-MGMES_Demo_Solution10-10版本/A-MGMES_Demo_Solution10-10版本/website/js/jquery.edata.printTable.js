    ...
    var timer = setInterval(function () {
        ...
        if (opt.table) {
            ...
            $.each($printArea, function (i, item) {
                var $_area = $(item)
                if ($_area.find('.datagrid-view').length == 0) {
                    var $tb = $_area.find("table.form").eq(0).clone().removeAttr("style");
                    $tb.find("th").css("width", "auto");
                    $tb.find("td").css("width", "auto");

                    $(win.document.body).append($tb);
                } else {
                    var $tb = $_area.find("table.datagrid-htable").clone().removeAttr("style");
                    var $data = $_area.find("table.datagrid-btable").find("tbody").clone();
                    var $title = $_area.find("div.grid-title");
                    var $subtitle = $_area.find("div.grid-subtitle");
                    var $summary = $_area.find("table.datagrid-ftable").find("tbody").clone();
